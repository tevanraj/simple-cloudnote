...
async function handleRequest(request, env) {
  ...
  if (path.startsWith("/api/image/")) {
    ...
    return handleGetImage(imageId, env);
  }
  ...
  if (path.startsWith("/api/note")) {
    ...
    if (noteId) {
      ...
      if (path.endsWith("/image") && request.method === "POST") {
          return handleImageUpload(noteId, request, env);
      }
    }
  }
...
async function handleGetNotes(request, env) {
  try {
    ...
    // Filter out objects that are not actual notes (like images)
    const noteObjects = listing.objects.filter(obj => !obj.key.includes('/'));
    
    const notes = await Promise.all(noteObjects.map(async obj => {
...
async function handleDeleteNote(noteId, env) {
  try {
    ...
    // First, delete all images associated with this note
    // by listing objects with the noteId prefix
    const imagesList = await bucket.list({ prefix: `${noteId}/` });
    
    // Delete all images in parallel
    if (imagesList.objects.length > 0) {
      await Promise.all(
        imagesList.objects.map(async (obj) => {
          await bucket.delete(obj.key);
          console.log(`Deleted image: ${obj.key}`);
        })
      );
    }
    ...
    return new Response(JSON.stringify({ 
      success: true,
      message: 'Note and associated images deleted'
    }), {
      headers: {
        "Content-Type": "application/json",
        ...corsHeaders
...
// Handle image upload
async function handleImageUpload(noteId, request, env) {
  try {
    // Check if note exists
    const bucket = env.NOTES;
    const noteExists = await bucket.head(noteId);
    ...
    const imageId = `${noteId}/img_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`;
    
    // Get file data
    const imageArrayBuffer = await imageFile.arrayBuffer();
    
    // Store image in R2
    await bucket.put(imageId, imageArrayBuffer, {
      httpMetadata: {
    ...
    const imageUrl = `${new URL(request.url).origin}/api/image/${imageId}`;
...
async function handleGetImage(imageId, env) {
  try {
    const bucket = env.NOTES;
    const image = await bucket.get(imageId);
    ...
    // Get content type from metadata, default to jpeg if not specified
